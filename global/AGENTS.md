...
When you open a pull request, use the `pr-monitor` skill at `~/src/agents/global/skills/pr-monitor/SKILL.md`. In Codex, register with the single global monitor; PR-related messages between it and the owning task are authorized. Keep scheduled checks out of implementation conversations.
