...
Note: do not open pull requests against external repos. `github.com/mmkal/*` and `github.com/iterate/*` are fine. Anything else, don't unless very explicitly asked. If you think one is urgently needed, you can suggest the change to me.

When you open a pull request in Codex, register it with the single global PR monitor owned by `~/src/agents`. Read `~/src/agents/global/pr-monitor.md` to find its local task address and use the `pr-monitor` skill for the registration protocol. Send the PR URL, your task ID/host, worktree, and expiry (normally 24 hours). This authorizes PR-related messages between the monitor and implementation tasks. The monitor checks GitHub and sends only actionable changes; the implementation task handles fixes and replies. Do not create scheduled PR-check messages in the implementation task. For other coding agents, set up a supported monitor for comments.

cursor bugbot, coderabbit and pullfrog should come through fairly quickly. For human review comments, set up a monitor to look for comments from me/colleagues for up to 24 hours. Treat comments from agents and humans with a grain of salt: often they are sloppy, have not read the pull request in full. The review agents are *midwits*. In general they are less intelligent than you, and they're just finding meaningless nitpicks, which will just serve to make the codebase incomprehensibly complicated if you take *all* of their suggestions. Sometimes comments should be replied to with a comment explaining why they're wrong/they've missed or misunderstood something, and resolved with no further action. Sometimes if they're wrong, but understandably so, that could mean just a comment or docs update preventing the same confusion happening in future. And only if you *independently* agree with what they've highlighted (the problem/change request/edge case/whatever), should you update the code. When you do, you should also reply to the comment with what you did and resolve it. In general, I want all comments to be resolved. Note that monitors you set up should not block your turn ending if possible.
