...
4. After all the above is done, worktreeify again with the `improve-codebase-architecture` skill, against the default branch or the bedtime base branch. If the skill surfaces candidate architecture changes and would normally pause for user choice, use a small `grill-you` on the candidate list to choose one candidate (optimise for highest impact improvement rather than lowest hanging fruit) and flesh it out into an implementation-ready task, record the assumptions in the task/PR, and keep going. Once you've made the improvements, this might create several conflicts with other open pull requests, including the ones from this bedtime run. Once improve-codebase-architecture is done, create alternate reimplementations of every open pull request so they don't go stale. Inherit already-made product/API design decisions from the open pull requests where appropriate. In trivial cases, lift the code from the old pull request directly. For each reimplementation, push the branch but don't create another pull request to avoid confusion. Instead add a table to the body of the improve-codebase-architecture pull request, mapping each old PR to a new compare link. That way in the morning I can review the compare links. If I'm happy with the improve-codebase-architecture run, I can tell you to merge it and then replace the old PRs with the improved compare links.
