...
Complete. Strict replay, update-mode source rewriting, per-step `onDispose` cleanup, a small Calculator demo, and a larger Cursor/TypeScript demo are implemented and tested.
...
- The "AI" planner should only be used in update mode to discover deterministic commands. Checked-in `how` recipes must stay fast and deterministic, and must not call an agent.
...
- [x] Implement `createDemoHelper(expect.getState())` with async disposal and enough test-state parsing to find the calling test file. _Added `src/demo-helper.ts`; async disposal writes pending update-mode recipes back to the test file._
...
- [x] Add per-step cleanup to demo recipes. _Added `onDispose` as a sibling of `how`; cleanup callbacks/commands are registered on the demo helper and run from its async disposer._
- [x] Remove extra demo env gates. _Examples now run when selected directly with `vitest run examples/...`; normal `pnpm test` is scoped to `tests/` so demos remain opt-in._
...
- 2026-05-29: Added `onDispose` cleanup recipes. Commands run through the normal runner with a `dispose` phase; callback disposers are also supported. Demo examples no longer need `RUN_DEMO_SMALL` or `RUN_DEMO_TSC_CURSOR`.
- 2026-05-29: Removed the broken `peekaboo agent` fallback from generated `how` recipes. The default planner now infers deterministic recipes for simple app-launch and Calculator steps, and update mode can fall back to a Pi planner that returns concrete shell commands while rejecting agent commands in the recipe.
- 2026-06-01: Added checked command postconditions. `demo.exec(...)` now supports `.check(({ stdout }) => ...)` and `.json().check((data) => ...)`, so postconditions can assert on command output instead of merely confirming that a command exited successfully.
